...
import { makeIntentTools } from "../intent/tools.ts";
...
function buildQqIntentTools(pi: ExtensionAPI, cwd: string): any[] {
  // qq is a non-interactive side-chat: it cannot prompt the user to
  // confirm worktree creation, so we omit the gate hook. The factory
  // falls back to creating/adopting the worktree directly when
  // lock_intent runs with createWorktree=true. Side-chat callers that
  // do not want a worktree can pass createWorktree=false.
  return makeIntentTools({
    pi,
    getCwd: () => cwd,
  });
}
